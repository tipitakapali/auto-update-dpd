...
def main(batch_size=1002):
    ...
    dpd_db_file = f"dpd_spliter_tipitakapali.db"
   
    print(f"\nGenerating SQLite databases: {dpd_db_file}")
    print("\nIt may take hours. Please wait...")

    delete_existing_databases(dpd_db_file)
    ...
    with open(input_tab_file, "r", encoding="utf-8") as f:
        for i, line in enumerate(f, 1):
            ...
            main_defi = f'<section class="{wrap_class}">{word_lead}{main_defi}</section>'
    ...
    print_database_sizes(dpd_db_file)
